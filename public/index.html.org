  ...
  <div class="container">
    <header style="display:flex;gap:16px;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;margin-bottom:12px;">
      <div>
        ...
        <div class="muted">空き4=青 / 空き3=緑 / 空き2=赤 / 空き1=灰 / 空き0=白（同一投稿者による同番号の複数取得は不可）</div>
      </div>
      <div class="legend">
        <div class="legend-item"><span class="legend-swatch" style="background:var(--blue);"></span><span>空き4</span></div>
        <div class="legend-item"><span class="legend-swatch" style="background:var(--green);"></span><span>空き3</span></div>
        <div class="legend-item"><span class="legend-swatch" style="background:var(--red);"></span><span>空き2</span></div>
        <div class="legend-item"><span class="legend-swatch" style="background:var(--gray);"></span><span>空き1</span></div>
        <div class="legend-item"><span class="legend-swatch" style="background:var(--white);border:1px solid #d1d5db;"></span><span>空き0</span></div>
      </div>
    </header>
    </header>

<div id="statusArea" class="card" style="margin-bottom:12px;padding:10px;display:flex;align-items:center;gap:12px;">
  <div><strong>現在のステータス：</strong> <span id="statusText">--</span></div>
  <div style="margin-left:auto;display:flex;gap:8px;">
    <button class="btn" onclick="setStatus('準備中')">準備中</button>
    <button class="btn" onclick="setStatus('お休み入力中')">お休み入力中</button>
    <button class="btn" onclick="setStatus('投稿待ち')">投稿待ち</button>
  </div>
</div>
...
<section class="card" style="margin-bottom:12px;">
  <div class="toolbar">
    ...
    <div class="field">
      <label>基地番号（<strong>カンマ区切りのみ</strong>）｜空き追加: <strong>スペース無しの「-」で開始</strong></label>
      <input id="inNums" type="text" placeholder="例: 5,12,27 ／ 空き追加例: -5,5,12" />
    </div>
    <div class="actions">
      <button class="btn btn-primary" id="btnPost">投稿</button>
      <button class="btn" id="btnReset">全リセット</button>
      <button class="btn btn-amber" id="btnMaint">お休み管理</button>
    ...
    <div id="maintBar" class="maint-bar">
      ...
      <div>🛠 <strong>お休み管理モード中</strong>：番号セルをクリックすると <strong>空き+1（最大4）</strong>。</div>
      <div><span class="muted">完了したら OFF に戻してください。</span></div>
    </div>
  </div>
</section>
...
<section class="row" style="gap:12px;">
  ...
  <div class="card">
    ...
    <div class="muted" style="margin-bottom:6px;">（確保済みの番号を投稿者ごとに集計表示）</div>
...
<section class="row" style="margin-top:12px;gap:12px;">
  <div class="card">
    ...
    <div class="logs" id="logs">
      <div class="muted">投稿はまだありません。</div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <ul style="margin:0 0 6px 0px;font-size:14px;color:#374151;">
      <li>基地取得：<strong>カンマ区切りのみ</strong>（空白不可）。例：<code>5,12,27</code></li>
      <li>空き追加：<strong>先頭をスペース無しの「-」で開始</strong>し、後は <strong>カンマ区切りのみ</strong>。例：<code>-5,5,12</code> → 5を+2、12を+1（最大4）。</li>
      <li><strong>「- 5,12」など、マイナスの直後にスペースがある投稿はエラー</strong>として拒否します。</li>
      <li>お休み管理モード：ONの間、セルクリックで空き+1（最大4）。</li>
      <li>全リセット：全番号を空き0に。</li>
    </ul>
  </div>
</section>
</div>
<div style="text-align:center; margin-top:24px;">
  <a href="./mapping.html" target="_blank"
     style="
...
(function(){
  ...
  function renderBoard(){
    for(let n=1;n<=BOARD_SIZE;n++){
      ...
      c.sub.textContent = rem === 0 ? '空き0' : '空き' + rem;
...
function formatTime(ts){
  ...
  function renderLogs(){
    ...
    if(logs.length === 0){
      const d=document.createElement('div');
      d.className='muted';
      d.textContent='投稿はまだありません。';
      elLogs.appendChild(d);
      return;
    }

    const currentUser = inUser.value.trim();
    ...
    for(const l of logs){
      ...
      head.textContent = `${formatTime(l.at - 400)} ・ ${l.user || '匿名'}`;

      /* ★ 投稿者本人だけに取消ボタンを表示 */
      if(currentUser && currentUser === l.user){
        const undoBtn = document.createElement('button');
        undoBtn.textContent = "取消";
        undoBtn.className = "btn";
        undoBtn.style.marginLeft = "8px";
        undoBtn.onclick = () => undoLog(l.id, l.user);
      ...
      for(const r of (l.results || [])){
        ...
        if(r.type === 'inc'){
          ...
          b.textContent = r.ok ? `${r.number} 取得` : `${r.number}${r.reason==='dup'?' 重複不可':' 番なし'}`;
        }
        body.appendChild(b);
      }
...
function renderStatus() {
  ...
  window.setStatus = async function(s) {
    if (!confirm(`ステータスを「${s}」に変更します。よろしいですか？`)) return;
  ...
  function renderUserAlloc(){
    ...
    for(const u of users){
      ...
      for(const n of map.get(u).slice().sort((x,y)=>x-y)){
        ...
        s.className = u.includes('お休み') ? 'n n-rest' : 'n';
  ...
  function parseInput(raw){
    ...
    if(/^-[\s]/.test(s)) return {mode:'err',numbers:[],err:'空き追加は「-」の直後にスペースを入れないでください（例: -5,12）'};
    if(s.startsWith('-')){
      const payload=s.slice(1);
      if(payload.includes(' ')) return {mode:'err',numbers:[],err:'空き追加はカンマのみで区切ってください（空白不可）'};
    ...
    if(/\s/.test(s)) return {mode:'err',numbers:[],err:'基地取得はカンマ区切りのみ使用できます（空白不可）'};
  ...
  async function post(){
    ...
    const body = { mode: parsed.mode, numbers: parsed.numbers, user: inUser.value || '匿名' };
  ...
  /* ============================
     ★ 投稿取り消し（ログから）
  ============================ */
  async function undoLog(id, user){
    if(!confirm("この投稿を取り消します。よろしいですか？")) return;
    ...
    const st = await res.json();
    if(st.error){
      elReply.innerHTML = `<span class="reply">${st.error}</span>`;
      return;
    }

    state = st;
    renderAll();
    elReply.innerHTML = `<span class="reply">投稿を取り消しました</span>`;
  }
  ...
  async function maintGroupInc(name){
    if (state.maintUsed.includes(name)) {
      elReply.innerHTML = `<span class="reply">${name} はすでに実行済みです</span>`;
      return;
    }
    ...
    renderAll();
    elReply.innerHTML = `<span class="reply">${name} の担当番号をすべて +1 しました</span>`;
  }
  ...
  function setMaint(on){
    ...
    btnMaint.textContent = maint ? 'お休み管理（ON）' : 'お休み管理';
  ...
  async function handleCellClick(n){
    ...
    if(rem === MAX_SLOTS){
      const res = await fetch('/post', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({ mode:'set0', numbers:[n], user:'SYSTEM' })
      });
      state = await res.json();
      renderAll();
      elReply.innerHTML='<span class="reply">'+n+' を空き0にしました (SYSTEM)</span>';
      return;
    }

    if(rem >= MAX_SLOTS){
      elReply.innerHTML='<span class="reply">'+n+' は既に空き4です</span>';
      return;
    }
    ...
    const res = await fetch('/post', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({ mode:'inc', numbers:[n], user:'SYSTEM' })
    });
    state = await res.json();
    renderAll();
    elReply.innerHTML='<span class="reply">'+n+' の空きを +1 しました (SYSTEM)</span>';
  }
